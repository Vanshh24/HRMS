    ...
    <main className="space-y-6">
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-3xl bg-white p-6 shadow-sm">
          <h3 className="text-lg font-semibold text-slate-900">Recent Hires</h3>
          <ul className="mt-4 space-y-3 text-sm text-slate-600">
            {recentHires.map(({ _id, employeeName }) => (
              <li key={_id} className="rounded-2xl bg-slate-50 px-4 py-3">
                {employeeName}
              </li>
            ))}
          </ul>
